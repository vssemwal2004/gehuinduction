...
function AdminDashboard() {
  ...
  return <div className="min-h-screen">
    ...
    <main className="lg:pl-60">
      <header className="border-b border-slate-200 bg-white px-5 py-4 sm:px-8"><div className="flex items-center justify-between gap-4"><div className="lg:hidden"><div className="text-sm font-bold text-blue-700">GEU Induction Connect</div><select value={activePage} onChange={(e) => setActivePage(e.target.value)} className="mt-1 h-8 rounded-md border border-slate-300 px-2 text-xs"><option>Dashboard</option><option>Students</option><option>Coordinators</option><option>Groups & WhatsApp</option><option>Activity Logs</option><option>Settings</option></select></div><div className="hidden lg:block"><div className="text-sm font-semibold text-slate-900">{activePage}</div><p className="mt-1 text-xs text-slate-500">GEU Induction Programme 2026</p></div><div className="text-right"><div className="text-sm font-semibold">{user.name}</div><div className="text-xs text-slate-500">{user.email}</div></div></div></header>
      {activePage === 'Groups & WhatsApp' ? <div className="p-5 sm:p-8"><GroupsPage/></div> : activePage === 'Students' ? <div className="p-5 sm:p-8"><StudentsPage/></div> : activePage === 'Coordinators' ? <div className="p-5 sm:p-8"><CoordinatorsPage/></div> : activePage === 'Activity Logs' ? <div className="p-5 sm:p-8"><OperationsPage/></div> : activePage === 'Settings' ? <div className="p-5 sm:p-8"><SettingsPage/></div> : <div className="space-y-6 p-5 sm:p-8">
