...
function Stat({ label, value, tone = 'default', onClick, actionLabel = 'View details' }) {
  const color = tone === 'green' ? 'text-emerald-600' : tone === 'amber' ? 'text-amber-600' : tone === 'red' ? 'text-red-600' : 'text-slate-900';
  const interactionColor = tone === 'amber' ? 'hover:border-amber-200 hover:bg-amber-50/30 focus:ring-amber-200' : 'hover:border-red-200 hover:bg-red-50/30 focus:ring-red-200';
  const className = `rounded-lg border border-slate-200 bg-white p-4 text-left shadow-sm ${onClick ? `cursor-pointer transition focus:outline-none focus:ring-2 ${interactionColor}` : ''}`;
  const content = <><div className="text-xs font-medium text-slate-500">{label}</div><div className={`mt-2 text-2xl font-semibold ${color}`}>{value}</div>{onClick ? <div className={`mt-2 text-[11px] font-medium ${tone === 'amber' ? 'text-amber-700' : 'text-red-600'}`}>{actionLabel}</div> : null}</>;
...
function AdminDashboard() {
  ...
  return <div className="min-h-screen min-w-0">
    <aside className="fixed inset-y-0 left-0 hidden w-60 border-r border-slate-200 bg-white lg:block">
      ...
      <nav className="space-y-1 p-3" aria-label="Admin navigation">{navItems.map(([label, Icon]) => <button key={label} onClick={() => { setShowFailedEmails(false); setStudentStatus(''); setActivePage(label); }} className={`flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-left text-sm font-medium ${activePage === label ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}><Icon size={17}/>{label}</button>)}</nav>
    ...
    <main className="min-w-0 lg:pl-60">
      ...
      {activePage === 'Admins' ? <div className="p-4 sm:p-6 xl:p-8"><AdminsPage/></div> : activePage === 'Groups & WhatsApp' ? <div className="p-4 sm:p-6 xl:p-8"><GroupsPage/></div> : activePage === 'Students' ? <div className="p-4 sm:p-6 xl:p-8"><StudentsPage key={studentStatus || 'all-students'} initialStatus={studentStatus}/></div> : activePage === 'Student QR Data' ? <div className="p-4 sm:p-6 xl:p-8"><StudentQrDataPage/></div> : activePage === 'Coordinators' ? <div className="p-4 sm:p-6 xl:p-8"><CoordinatorsPage/></div> : activePage === 'Activity Logs' ? <div className="p-4 sm:p-6 xl:p-8"><OperationsPage key={showFailedEmails ? 'failed-mail' : 'activity'} initialTab={showFailedEmails ? 'mail' : 'activity'} initialStatus={showFailedEmails ? 'failed' : ''}/></div> : activePage === 'Settings' ? <div className="p-4 sm:p-6 xl:p-8"><SettingsPage/></div> : <div className="space-y-5 p-4 sm:p-6 xl:p-8">
        ...
        {dashboard?.courseStats?.length ? <section className="rounded-lg border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-4"><h2 className="text-sm font-semibold">Course-wise registration</h2></div><div className="overflow-x-auto"><table className="w-full min-w-[620px] text-left"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-4 py-3">Course</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Registered</th><th className="px-4 py-3">Pending</th><th className="px-4 py-3">Completion</th></tr></thead><tbody className="divide-y divide-slate-100">{dashboard.courseStats.map((item) => <tr key={item.course} className="text-sm"><td className="px-4 py-3 font-medium">{item.course}</td><td className="px-4 py-3">{item.totalStudents}</td><td className="px-4 py-3 text-emerald-700">{item.registeredStudents}</td><td className="px-4 py-3 text-amber-700">{item.pendingStudents}</td><td className="px-4 py-3">{item.registrationPercent}%</td></tr>)}</tbody></table></div></section> : null}
      </div>}
    </main>
  </div>;
}
