...
const emptyForm = { name: '', email: '', mobile: '', role: 'group_coordinator' };
...
function CoordinatorForm({ initialValue, onClose, onSaved }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4" role="dialog" aria-modal="true"><form onSubmit={submit} className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-xl">
    <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="text-base font-semibold">{initialValue ? 'Edit coordinator' : 'Add coordinator'}</h2><p className="mt-1 text-xs text-slate-500">Choose access type carefully. Scan coordinators only receive scanner access.</p></div><button type="button" onClick={onClose} className="rounded-md p-2 hover:bg-slate-100" aria-label="Close"><X size={17}/></button></div>
    <div className="grid gap-4 p-5 sm:grid-cols-2">{error ? <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700 sm:col-span-2">{error}</div> : null}
      ...
      <label className="sm:col-span-2"><span className="text-xs font-medium">Coordinator type</span><select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="mt-1.5 h-10 w-full rounded-md border border-slate-300 px-3 text-sm"><option value="group_coordinator">Group coordinator</option><option value="scan_coordinator">Scan coordinator</option></select></label>
    </div>
...
export default function CoordinatorsPage() {
  ...
  useEffect(() => { const timer = setTimeout(load, 200); return () => clearTimeout(timer); }, [search, role, status]); // eslint-disable-line react-hooks/exhaustive-deps
  const counts = useMemo(() => ({
    total: items.length,
    group: items.filter((item) => item.role === 'group_coordinator').length,
    scan: items.filter((item) => item.role === 'scan_coordinator').length,
  }), [items]);
  ...
  return <div className="space-y-4">
    <div className="flex items-end justify-between gap-3"><div><h1 className="text-xl font-semibold">Coordinators</h1><p className="mt-1 text-xs text-slate-500">Manage group ownership and mobile QR scanner access.</p></div><div className="flex gap-2"><button onClick={() => setBulkOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-md border border-slate-300 px-3 text-sm font-semibold"><Upload size={15}/>Bulk import</button><button onClick={() => setForm(emptyForm)} className="inline-flex h-9 items-center gap-2 rounded-md bg-blue-600 px-3 text-sm font-semibold text-white"><Plus size={16}/>Add coordinator</button></div></div>
    <div className="grid gap-3 sm:grid-cols-3">{[['Visible records', counts.total], ['Group coordinators', counts.group], ['Scan coordinators', counts.scan]].map(([label, value]) => <div key={label} className="rounded-lg border border-slate-200 bg-white p-4"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 text-xl font-semibold">{value}</div></div>)}</div>
