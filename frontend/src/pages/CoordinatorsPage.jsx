...
function CoordinatorForm({ initialValue, onClose, onSaved }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4" role="dialog" aria-modal="true"><form onSubmit={submit} className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-xl">
    ...
    <div className="grid gap-4 p-5">{error ? <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-700">{error}</div> : null}
      ...
      <div className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700">A secure random password will be emailed automatically.</div>
    </div>
...
export default function CoordinatorsPage() {
  ...
  return <div className="scan-coordinators-page space-y-4">
    <div className="flex items-end justify-between gap-3"><div><h1 className="text-xl font-semibold">Scan Coordinators</h1><p className="mt-1 text-xs text-slate-500">Manage accounts that can verify one-time gate entry.</p></div><button onClick={() => setForm(emptyForm)} className="inline-flex h-9 items-center gap-2 rounded-md bg-fuchsia-600 px-3 text-sm font-semibold text-white"><Plus size={16}/>Add scan coordinator</button></div>
    ...
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm"><div className="grid gap-3 border-b border-slate-200 p-4 md:grid-cols-[1fr_180px]"><label className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400"/><input value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 w-full rounded-md border border-slate-300 pl-9 pr-3 text-sm" placeholder="Search name, email or mobile"/></label><select value={status} onChange={(e) => setStatus(e.target.value)} className="h-9 rounded-md border border-slate-300 px-3 text-sm"><option value="">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead className="bg-slate-50 text-[11px] uppercase text-slate-500"><tr><th className="px-4 py-3">Coordinator</th><th className="px-4 py-3">Mobile</th><th className="px-4 py-3">Last login</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan="5" className="p-10 text-center text-sm text-slate-500">Loading coordinators…</td></tr> : items.length ? items.map((item) => <tr key={item._id} className="text-sm"><td className="px-4 py-3"><div className="font-semibold">{item.name}</div><div className="text-xs text-slate-500">{item.email}</div></td><td className="px-4 py-3 text-slate-600">{item.mobile}</td><td className="px-4 py-3 text-xs text-slate-500">{item.lastLoginAt ? new Date(item.lastLoginAt).toLocaleString() : 'Never'}</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-xs font-medium ${item.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{item.isActive ? 'Active' : 'Inactive'}</span></td><td className="px-4 py-3 text-right"><button onClick={(event) => openCoordinatorMenu(item, event)} className="rounded-md p-2 hover:bg-slate-100" aria-label={`Actions for ${item.name}`}><MoreVertical size={16}/></button></td></tr>) : <tr><td colSpan="5" className="p-10 text-center text-sm text-slate-500">No scan coordinators found.</td></tr>}</tbody></table></div>
    </div>
