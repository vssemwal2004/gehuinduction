...
const emptyForm = { name: '', studentId: '', email: '', groupIds: [], groupCoordinatorName: '', groupCoordinatorMobile: '', qrDataPhone: '' };

function StudentForm({ groups, initialValue, requireGroupDetails, onClose, onSaved }) {
...
export default function StudentsPage({ initialStatus = '' }) {
  ...
  const [options, setOptions] = useState({ requireGroupDetails: true });
  ...
  async function load(page = pagination.page) {
    ...
    try {
      ...
      setOptions(data.options || { requireGroupDetails: true });
  ...
  return <div className="space-y-4">
    ...
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-left"><thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3 font-semibold">Student</th><th className="px-4 py-3 font-semibold">Student ID</th><th className="px-4 py-3 font-semibold">Phone</th><th className="px-4 py-3 font-semibold">Group</th><th className="px-4 py-3 font-semibold">Coordinator</th><th className="px-4 py-3 font-semibold">Status</th><th className="px-4 py-3 font-semibold">Last scan</th><th className="px-4 py-3 text-right font-semibold">Actions</th></tr></thead><tbody className="divide-y divide-slate-100">{loading ? <tr><td colSpan="8" className="px-4 py-12 text-center text-sm text-slate-500">Loading students…</td></tr> : students.length ? students.map((student) => <tr key={student._id} className="text-sm hover:bg-slate-50/70"><td className="px-4 py-3"><div className="font-semibold text-slate-900">{student.name}</div><div className="mt-0.5 text-xs text-slate-500">{student.email}</div></td><td className="px-4 py-3 text-slate-600">{student.studentId}</td><td className="px-4 py-3 text-slate-600">{student.hasStudentQrData ? student.qrDataPhone : '—'}</td><td className="px-4 py-3 text-slate-600">{student.groupIds?.map((group) => group.code).join(', ') || '—'}</td><td className="px-4 py-3"><div className="text-slate-700">{student.groupCoordinatorId?.name || 'Not assigned'}</div><div className="text-xs text-slate-500">{student.groupCoordinatorId?.mobile || ''}</div></td><td className="px-4 py-3"><StatusBadge student={student}/></td><td className="px-4 py-3 text-xs text-slate-600">{student.lastScannedAt ? new Date(student.lastScannedAt).toLocaleString() : 'Never'}</td><td className="px-4 py-3 text-right"><button onClick={(event) => openStudentMenu(student, event)} className="rounded-md p-2 text-slate-500 hover:bg-slate-100" aria-label={`Actions for ${student.name}`} aria-expanded={openMenu?.studentId === student._id}><MoreVertical size={16}/></button></td></tr>) : <tr><td colSpan="8" className="px-4 py-12 text-center text-sm text-slate-500">No students found.</td></tr>}</tbody></table></div>
    ...
    {formOpen ? <StudentForm groups={groups} initialValue={editing} requireGroupDetails={options.requireGroupDetails} onClose={() => setFormOpen(false)} onSaved={() => { setFormOpen(false); load(1); }}/> : null}
